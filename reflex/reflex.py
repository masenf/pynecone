...
def run(
    ...
    db_migrate: bool = typer.Option(
        True,
        "--no-db-migrate",
        help="Disable migration of database schema.",
    ),
):
    ...
    # Update db migration settings
    if not db_migrate:
        get_config().db_migrate = False
...
def export(
    ...
    if frontend:
        # never migrate the database during export
        config.db_migrate = False
...
def migrate():
    ...
    # explicitly running `reflex db migrate` overrides config file value
    get_config().db_migrate = True
    prerequisites.get_app()
    prerequisites.check_schema_up_to_date()
...
def makemigrations():
    ...
    if not model.has_alembic:
        console.print(
            "[red]Alembic is not installed. Run [bold]pip install alembic[/bold] first."
        )
        return
    if not Path(constants.ALEMBIC_CONFIG).exists():
        console.print(
            "[red]Alembic is not initialized. Run [bold]python -m alembic init alembic[/bold] first."
        )
        return
    get_config().db_migrate = False
    prerequisites.get_app()
