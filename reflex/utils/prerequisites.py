...
def check_schema_up_to_date():
    ...
    with model.Model.get_db_engine().connect() as connection:
        ...
        try:
            if model.Model.alembic_autogenerate(connection=connection, write_migration_scripts=False):
                console.print(
                    "[red]Detected database schema changes. Run [bold]reflex db makemigrations[/bold] to generate migration scripts."
                )
        except CommandError as command_error:
            if "Target database is not up to date." in str(command_error):
                console.print(f"[red]{command_error} Run [bold]reflex db migrate[/bold] to update database.")
            else:
                raise
